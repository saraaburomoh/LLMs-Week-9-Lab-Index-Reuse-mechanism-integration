...
> ⚠️ Use **`uv`** only. Do not use `pip install`.

### 2. Install the pre-built ChromaDB index
...
Verify: `chroma.sqlite3` (~4.6 GB) should exist at the target path.
...
# Process mode: hierarchical | sequential | collaborative
...
FLOW_STAGGER_SECONDS=45
FLOW_WEB_WAIT_SECONDS=30
FLOW_PREDICTION_WAIT_SECONDS=60
PREDICTION_MAX_CTX_CHARS=1200
```

> `minimaxai/minimax-m2.7` is currently a **free endpoint** on NVIDIA Build. Register at [https://build.nvidia.com/](https://build.nvidia.com/).
...
Expected output (each query under 1 second):
```
...
# Sequential / collaborative mode
uv run first_crew sequential
```

**Output:** `hierarchical_report.json` (or `sequential_report.json`):
```json
{
  "stars": 4.0,
  "review": "..."
...
→ Regenerate your key at [https://build.nvidia.com/](https://build.nvidia.com/) and update `.env`.

**`An instance of Chroma already exists with different settings`**
→ Do not run multiple scripts simultaneously. Restart terminal and retry.

**`benchmark_rag.py` passes but agents report "data not found"**
→ Verify `config/tasks.yaml` uses `search_query` style descriptions for RAG tools, and that exact IDs are passed to the `lookup_*` tools.

**Queries taking >10 seconds in `benchmark_rag.py`**
→ `chroma.sqlite3` is in the wrong directory. Re-check [Setup Step 2](#2-install-the-pre-built-chromadb-index).

**Re-build the index from scratch (1–4 hours on CPU):**
```bash
